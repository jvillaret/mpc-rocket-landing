"""
MPCControl_yvel - Y-velocity subsystem controller for Deliverable 4.1

This controller uses SOFT CONSTRAINTS with slack variables to handle model mismatch.
Tuning parameters:
    - Q = diag([200.0, 100.0, 10.0]) for states [wx, alpha, vy]
    - R = [100.0] for input [d1]
    - Input constraints: ±15° (hard constraints)
    - State constraints: alpha within ±10° (SOFT constraints with slack variables)
...
class MPCControl_yvel(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        # Slack variables for soft state constraints (alpha)
        # One slack variable per time step for upper and lower bounds
        self.epsilon_alpha_pos = cp.Variable(self.N + 1, nonneg=True, name='epsilon_alpha_pos')
        self.epsilon_alpha_neg = cp.Variable(self.N + 1, nonneg=True, name='epsilon_alpha_neg')
        ...
        # Q = diag([wx_weight, alpha_weight, vy_weight])
        Q = np.diag([100.0, 0.1, 0.1])  # Weights for [wx, alpha, vy]
        # Increased weight on wx (200.0) to strongly penalize aggressive rotational maneuvers
        # Increased weight on alpha (100.0) to very strongly penalize large angles
        # Moderate weight on vy (10.0) for velocity tracking
        
        R = np.diag([1.0])  # Weight for input [d1]
        ...
        for k in range(self.N):
            ...
            cost += rho_slack * (cp.square(self.epsilon_alpha_pos[k + 1]) + cp.square(self.epsilon_alpha_neg[k + 1]))
            cost += rho_slack * (self.epsilon_alpha_pos[k + 1] + self.epsilon_alpha_neg[k + 1])
            ...
            # State constraints: alpha (index 1 in [wx, alpha, vy]) - SOFT CONSTRAINTS
            # Use original constraint limit (±10°) but allow violations through slack variables
            alpha_max = np.deg2rad(9.9)
            # Soft upper bound: alpha <= 10° + epsilon_pos (epsilon_pos >= 0)
            constraints.append(self.x_var[1, k + 1] <= alpha_max + self.epsilon_alpha_pos[k + 1])
            # Soft lower bound: alpha >= -10° - epsilon_neg (epsilon_neg >= 0)
            constraints.append(self.x_var[1, k + 1] >= -alpha_max - self.epsilon_alpha_neg[k + 1])

        # Terminal cost
        cost += cp.quad_form(self.x_var[:, self.N] - self.x_target[:, self.N], Q)
        
        # Penalty for terminal slack variables
        cost += rho_slack * (cp.square(self.epsilon_alpha_pos[self.N]) + cp.square(self.epsilon_alpha_neg[self.N]))
        cost += rho_slack * (self.epsilon_alpha_pos[self.N] + self.epsilon_alpha_neg[self.N])
        
        # Terminal state constraint: alpha - SOFT CONSTRAINT at terminal step
        alpha_max = np.deg2rad(10.0)
        constraints.append(self.x_var[1, self.N] <= alpha_max + self.epsilon_alpha_pos[self.N])
        constraints.append(self.x_var[1, self.N] >= -alpha_max - self.epsilon_alpha_neg[self.N])
