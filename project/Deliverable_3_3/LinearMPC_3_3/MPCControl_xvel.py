...
class MPCControl_xvel(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        # Slack variable for soft constraint on beta (state index 1 in local coordinates)
        self.epsilon_beta = cp.Variable(self.N + 1, name='epsilon_beta')
        ...
        # Penalty weight for slack variable (soft constraint on beta)
        ...
        rho_beta = 50.0  # Penalty weight for slack variable - soft constraint (not hard)
        ...
        # Beta constraint limit: 10 degrees
        ...
        for k in range(self.N):
            ...
            cost += rho_beta * self.epsilon_beta[k]
            ...
            # Soft constraint on beta: |beta| <= beta_max + epsilon_beta
            # beta is at index 1 in local state vector
            constraints.append(self.x_var[1, k] <= beta_max + self.epsilon_beta[k])
            constraints.append(self.x_var[1, k] >= -beta_max - self.epsilon_beta[k])
            
            # Slack variable must be non-negative
            constraints.append(self.epsilon_beta[k] >= 0)

        # Terminal cost (LQR-based Qf for stability)
        cost += cp.quad_form(self.x_var[:, self.N] - self.x_target[:, self.N], Qf)
        
        # Terminal soft constraint on beta
        cost += rho_beta * self.epsilon_beta[self.N]
        constraints.append(self.x_var[1, self.N] <= beta_max + self.epsilon_beta[self.N])
        constraints.append(self.x_var[1, self.N] >= -beta_max - self.epsilon_beta[self.N])
        constraints.append(self.epsilon_beta[self.N] >= 0)
