...
class MPCControl_roll(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        # Q: [wz, gamma] - Higher Q[1] drives gamma to zero faster
        # R: [Pdiff] - Lower R allows more aggressive control
        # Recommended tuning for settling from 40° to 0 in <7s:
        Q = np.diag([1.0, 10.0])  # Weights for [wz, gamma] - increase Q[1] for faster roll settling
        R = np.diag([0.5])        # Weight for input [Pdiff] - lower R allows more control effort
        ...
        # No state constraints on gamma (roll subsystem valid for any roll angle)
        # Use unconstrained state space (large box)
        F_x = np.vstack([np.eye(self.nx), -np.eye(self.nx)])
        f_x = np.hstack([100.0 * np.ones(self.nx), 100.0 * np.ones(self.nx)])
        X = Polyhedron.from_Hrep(F_x, f_x)

        # Input constraints: |Pdiff| <= 20%
        F_u = np.array([[1], [-1]])
        f_u = np.array([20.0, 20.0])
        ...
        for k in range(self.N):
            ...
            # Input constraints (Pdiff +/- 20%)
            constraints.append(self.u_var[:, k] <= 20.0)
            constraints.append(self.u_var[:, k] >= -20.0)
    ...
    def get_u(
        ...
        if self.ocp.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
            ...
            print(f"MPC Solver failed (Roll): status={self.ocp.status}, returning trim values.")
            u0 = self.us
