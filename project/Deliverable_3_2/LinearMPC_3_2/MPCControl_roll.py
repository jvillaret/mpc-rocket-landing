...
class MPCControl_roll(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        Q = np.diag([1.0, 10.0])  # Weights for [wz, gamma]
        R = np.diag([0.5])        # Weight for input [Pdiff]

        # Compute LQR terminal cost and gain
        K, Qf, _ = dlqr(self.A, self.B, Q, R)
        K = -K

        # State constraints: |wz|<=180d/s, |gamma|<=15deg
        F_x = np.array([
            [1, 0], [-1, 0],
            [0, 1], [0, -1]
        ])
    ...
    def get_u(
        ...
        if self.u_var.value is not None:
            ...
            print("MPC Solver failed (Roll), returning trim values.")
            u0 = self.us
